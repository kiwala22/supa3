...
RSpec.describe DrawWorker, type: :worker do
    ...
    it "runs normally when the draw ID has a reminder on modulo 3" do
        ...
        expect {
            DrawWorker.perform_async(draw_id, START_TIME, END_TIME)
        }.to change(DrawWorker.jobs, :size).by(1)
