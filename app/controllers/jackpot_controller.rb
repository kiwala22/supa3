class JackpotController < ApplicationController
   ...
   def supa_five_jackpot
      ...
      @winner = Ticket.where("created_at >= ? and created_at <= ? AND game = ? AND data = ?", start_date, end_date, "Supa5", winning_number).order("RANDOM()").first
      @match_count = Ticket.where("created_at >= ? and created_at <= ? AND game = ? AND data = ?", start_date, end_date, "Supa5", winning_number).count()
