  ...
  def index
    ...
    (0..30).each do |f|
      ...
      @series.each do |f|
        ...
        f[:gross_revenue] = f[:gross_revenue] > 0 ? ((ref_obj[:gross_revenue] - f[:gross_revenue]) /  f[:gross_revenue]) : 0
        #f[:gross_change] = (ref_obj[:gross_revenue] - f[:gross_revenue]) /  f[:gross_revenue]
        f[:rtp_change] = f[:rtp] > 0 ? ((ref_obj[:rtp] - f[:rtp]) /  f[:rtp]) : 0
        #f[:rtp_change] = (ref_obj[:rtp] - f[:rtp]) /  f[:rtp]
        f[:users_change] = f[:users] > 0 ? ((ref_obj[:users] - f[:users]) /  f[:users]) : 0
        #f[:users_change] = (ref_obj[:users] - f[:users]) /  f[:users]
        f[:new_users_change] = f[:new_users] > 0 ? ((ref_obj[:new_users] - f[:new_users]) /  f[:new_users]) : 0
