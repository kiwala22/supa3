class DisbursementsMailer < ApplicationMailer
   ...
   default to: ["Okello Hassan <acaciabengo@gmail.com>"]
	...
	def daily_report(csv)
		attachments["Disbursements Report - #{Date.yesterday.strftime('%d/%m/%Y')}"] = {mime_type: 'text/csv', content: csv}
		mail subject: "Disbursements Report for #{Date.yesterday.strftime('%d/%m/%Y')}"
	end
end
