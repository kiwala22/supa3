module MobileMoney
	class MtnEcw
		...
		# 	url = "https://f5.mtn.co.ug:8006/poextvip/v1/debit"
		...
		# 	url = "https://f5.mtn.co.ug:8006/poextvip/v1/refund"
		...
		def self.get_collection_balance
			url = "https://f5.mtn.co.ug:8006/poextvip/v1/getbalance"
			#fri = @@fri
			...
			http.verify_mode = OpenSSL::SSL::VERIFY_NONE
			http.cert = OpenSSL::X509::Certificate.new(File.read(Rails.root.join("config/prod/www.admin.supa3.info.crt")))
			hhttp.key = http.key = OpenSSL::PKey::RSA.new(File.read(Rails.root.join("config/prod/www.admin.supa3.info.key")))
			http.ca_file = Rails.root.join("config/prod/m3_EXTERNAL_ca_UG.crt").to_s
			#http.set_debug_output($stdout)
			res = http.request(request)
			result = Hash.from_xml(res.body)
